"""
Fitness Schema v3 + drug-response
==================================

Mirrors ``schema_v3.py`` but reserves 30 % of the fitness for a ``drug_response``
component that scores simulated post/pre rate ratios against the experimental
ratios stamped into ``/drug_effects/<drug>/`` of the reference h5 by
``_scripts/extract_drug_effects.py``.

All schema_v3 component weights have been scaled by 0.70 so the original
relative weighting (unit_metrics dominant, burstlets > pre_burstlets >
network_bursts, etc.) is preserved while leaving 0.30 for ``drug_response``.

Used by ``fitnessFunc_v2.py`` when ``run_batch.py --drugs <name>`` is invoked.
"""
...
# =============================================================================
# FITNESS SCHEMA — drug-augmented variant of schema_v3
# =============================================================================
...
fit_schema = {
    ...
    # ------------------------------------------------------------------
    'unit_metrics': {
        'weight': 0.35,  # was 0.50 in schema_v3 (× 0.70)
    ...
    # ------------------------------------------------------------------
    'burstlets': {
        'weight': 0.14,  # was 0.20 in schema_v3 (× 0.70)
    ...
    'network_bursts': {
        'weight': 0.07,  # was 0.10 in schema_v3 (× 0.70)
    ...
    # ------------------------------------------------------------------
    # 7. Drug-response ratios (NEW — post/pre simulated vs experimental)
    # ------------------------------------------------------------------
    # For each --drugs <name> given to run_batch.py, the simulator produces a
    # post-drug spike train alongside the baseline. fitnessFunc_v2 derives the
    # post/pre rate ratio for each metric listed here and compares it against
    # /drug_effects/<name>/<metric>_ratio in the reference target h5
    # (stamped by _scripts/extract_drug_effects.py).
    #
    # ``max_dev`` is the deviation magnitude at which a single drug-metric
    # contributes its full (= 1.0) loss; smaller deviations scale linearly.
    'drug_response': {
        'weight': 0.30,
        'description': 'Simulated post/pre rate ratios vs experimental drug ratios',
        'metrics': {
            'pop_FR_ratio': {
                'weight': 0.25,
                'max_dev': 1.0,
            ...
            'pre_burstlet_rate_ratio': {
                'weight': 0.15,
                'max_dev': 1.5,
                'description': 'Pre-burstlet rate ratio (post/pre); detector static=15 '
                               '(matches extract_drug_effects PRE_BURST_KW)',
            },
            'superburst_rate_ratio': {
                'weight': 0.10,
                'max_dev': 2.0,
                'description': 'Superburst rate ratio (post/pre); detector static=40',
            },
        },
    },
}
...
if __name__ == "__main__":
    print("=" * 60)
    print("FITNESS SCHEMA v3 (drug-augmented) SUMMARY")
    print("=" * 60)
    ...
    for cname, comp in fit_schema.items():
        ...
        for mname, m in comp['metrics'].items():
            ...
            print(f"    {mname:30s} w={m['weight']:.2f}  {bounds}")
