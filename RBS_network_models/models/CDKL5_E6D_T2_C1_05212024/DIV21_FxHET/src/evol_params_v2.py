...
if version == 2.0:
    ...
    params['propVelocity'] = [0.0, 0.3]  # Conduction velocities in mammalian neurons range from ~0.1 m/s in unmyelinated fibers to over 100 m/s in large myelinated axons. [Source: Wikipedia - Axon](https://en.wikipedia.org/wiki/Axon)
    # NOTE:
        # Propagation Parameters
    ...
    params.update({
        ...
        'E_diam_stdev': [0, 500],  # Dendritic diameters in excitatory neurons can vary significantly, with standard deviations up to 10 µm observed in cortical pyramidal cells. [Source: Nature - Morphological diversity of single neurons](https://www.nature.com/articles/s41586-021-03941-1)
        'E_L_stdev': [0, 500],  # Dendritic lengths in excitatory neurons show variability, with standard deviations reaching up to 100 µm, reflecting diverse branching patterns. [Source: Nature - Morphological diversity of single neurons](https://www.nature.com/articles/s41586-021-03941-1)
        'E_Ra_stdev': [0, 200],  # Axial resistance (Ra) variability in excitatory neurons can have standard deviations up to 50 Ω·cm, influencing signal propagation. [Source: PMC - Neuronal Morphology Enhances Robustness](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9974411/)

        'I_diam_stdev': [0, 60],  # Inhibitory interneurons exhibit dendritic diameter variability with standard deviations up to 5 µm. [Source: Nature - Morphological diversity of single neurons](https://www.nature.com/articles/s41586-021-03941-1)
        'I_L_stdev': [0, 500],  # Dendritic length variability in inhibitory neurons can have standard deviations up to 50 µm. [Source: Nature - Morphological diversity of single neurons](https://www.nature.com/articles/s41586-021-03941-1)
        'I_Ra_stdev': [0, 100],  # Axial resistance variability in inhibitory neurons can have standard deviations up to 40 Ω·cm. [Source: PMC - Neuronal Morphology Enhances Robustness](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9974411/)
        ...
        'E_diam_mean': [5, 800],  # Mean diameter of excitatory neurons (µm). Pyramidal cells average around 20 µm, with variability depending on species and brain region. [Source: Pyramidal cell](https://en.wikipedia.org/wiki/Pyramidal_cell)
        'E_L_mean': [50, 1500],  # Mean length of excitatory neurons (µm). Individual dendrites are usually several hundred micrometers long. [Source: Pyramidal cell](https://en.wikipedia.org/wiki/Pyramidal_cell)
        'E_Ra_mean': [70, 500],  # Mean axial resistance of excitatory neurons (Ω·cm). Axial resistance is inversely related to the cross-sectional area of the neuron. [Source: Length constant](https://en.wikipedia.org/wiki/Length_constant)

        'I_diam_mean': [4, 50],  # Mean diameter of inhibitory neurons (µm). Inhibitory interneurons have soma diameters averaging around 10 µm, with some variability. [Source: Golgi cell](https://en.wikipedia.org/wiki/Golgi_cell)
        'I_L_mean': [50, 1000],  # Mean length of inhibitory neurons (µm). Inhibitory neurons typically have shorter dendrites compared to excitatory neurons.
        'I_Ra_mean': [80, 800],  # Mean axial resistance of inhibitory neurons (Ω·cm). Due to their generally smaller diameters, inhibitory neurons exhibit higher axial resistance. [Source: Length constant](https://en.wikipedia.org/wiki/Length_constant)
    ...
    params.update({
        ...
        'gnabar_E': [0, 50],  # Widening the range significantly to account for variability and unknowns in sodium conductance for excitatory neurons.
        ...
        'gnabar_E_std': [0, 50],  # Widening the range significantly to account for variability and unknowns in standard deviation of sodium conductance.
        ...
        'gkbar_E': [0, 50],  # Widening the range significantly to account for variability and unknowns in potassium conductance for excitatory neurons.
        ...
        'gkbar_E_std': [0, 50],  # Widening the range significantly to account for variability and unknowns in standard deviation of potassium conductance.
        ...
        'gnabar_I': [0, 50],  # Widening the range significantly to account for variability and unknowns in sodium conductance for inhibitory neurons.
        ...
        'gnabar_I_std': [0, 50],  # Widening the range significantly to account for variability and unknowns in standard deviation of sodium conductance.
        ...
        'gkbar_I_std': [0, 50],  # Widening the range significantly to account for variability and unknowns in standard deviation of potassium conductance.
    })
    ...
    params.update({
        ...
        'tau1_exc': [0.1, 500],  # Rise time of excitatory synaptic conductance can range from 0.1 to 10 ms, depending on receptor types and synaptic dynamics. [Source: Estimating the Time Course of the Excitatory Synaptic Conductance](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6793890/)
        ...
        'tau2_exc': [0.1, 1500],  # Decay time of excitatory synaptic conductance can range from 1 to 100 ms, depending on receptor types and synaptic dynamics. [Source: Estimating the Time Course of the Excitatory Synaptic Conductance](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6793890/)
        ...
        'tau1_inh': [0.1, 500],  # Rise time of inhibitory synaptic conductance can range from 0.3 to 10 ms, depending on GABA_A and GABA_B receptor contributions. [Source: Neurotransmitter Time Constants (PSCs)](https://compneuro.uwaterloo.ca/research/constants-constraints/neurotransmitter-time-constants-pscs.html)
        ...
        'tau2_inh': [0.1, 1500],  # Decay time of inhibitory synaptic conductance can range from 5 to 100 ms, depending on GABA_A and GABA_B receptor contributions. [Source: Neurotransmitter Time Constants (PSCs)](https://compneuro.uwaterloo.ca/research/constants-constraints/neurotransmitter-time-constants-pscs.html)
    })
